...
public class WheelView extends View {
	...
	private static final int ADDITIONAL_ITEM_HEIGHT = DataMgr.dip2px(20);
	...
	private int Additional_Item_Height = ADDITIONAL_ITEM_HEIGHT;
	...
	public void setAdditional_Item_Height(int Additional_Item_Height) {
		this.Additional_Item_Height = Additional_Item_Height;
	}
	...
	private void initResourcesIfNecessary() {
		...
		if (spliteLineDrawable == null) {
			spliteLineDrawable = getContext().getResources().getDrawable(
					R.drawable.wheel_splite_line);
		}
...
//				- Additional_Item_Height;
	 ...
	 * @return the text
	 */
	private String buildText(boolean useCurrentValue, int width) {
		...
		for (int i = currentItem - addItems; i <= currentItem + addItems; i++) {
			if (useCurrentValue || i != currentItem) {
				...
				if (text != null) {
					if (text.length() > 20) {
						text = getEllipsizeString(text, itemsPaint, width);
					}
	...
	private void createLayouts(int widthItems, int widthLabel) {
		...
		if (!isScrollingPerformed
			...
			if (text == null) {
				...
				text = getEllipsizeString(text, valuePaint, widthItems);
			}
	...
	private String getEllipsizeString(String text, TextPaint paint, int width) {
		int bufferWidth =(int) paint.getTextSize()*1;
		int availableTextWidth = width * 1- bufferWidth;
...
//			canvas.translate(0, bounds.top + scrollingOffset);
			canvas.translate(0, bounds.top + Additional_Item_Height/2 + scrollingOffset);
...
//		canvas.translate(0, -top + scrollingOffset);
		canvas.translate(0, -top + Additional_Item_Height/2 + scrollingOffset);
...
//		centerDrawable.draw(canvas);
		spliteLineDrawable.setBounds(0, center - offset, getWidth(), center - offset + 1);
		spliteLineDrawable.draw(canvas);
		spliteLineDrawable.setBounds(0, center + offset - 1, getWidth(), center + offset);
		spliteLineDrawable.draw(canvas);
	}
	...
	private SimpleOnGestureListener gestureListener = new SimpleOnGestureListener() {
		...
		public boolean onSingleTapUp(MotionEvent e) {
			float y = e.getY();
			int index = (int) (y/getItemHeight());
			int top_bottom_item = visibleItems / 2;
			int position = index - top_bottom_item;
			if (position > top_bottom_item) {
				position = top_bottom_item;
			}

			setCurrentItem(getCurrentItem() + position, true);
			return true;
		}
	};
