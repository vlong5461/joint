...
public class ForgetPasswordActivity extends BaseActivity implements OnClickListener, BaseApi.APIListener {
	
	private EditText etUserName, etVerificationCode, etPassword;
	private Button btnGetVerificationCode, btnModify;
	...
	private void initView() {
		...
		btnModify = findView(R.id.btnModify);
	...
	private void modifyPassword() {
		...
		if (validate.length() == 0) {
			ToastUtils.show(mContext, getString(R.string.register_input_phone_code));
			return;
		}
		if (password.length() == 0) {
			ToastUtils.show(mContext, getString(R.string.register_input_password));
			return;
		} else {
		...
		api.requestCode = Consts.RequestCode.ModifyPassword;
		api.sendRequest(validate, mobile, password); //, "1");
