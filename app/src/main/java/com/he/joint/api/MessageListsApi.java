...
public class MessageListsApi extends BaseApi {

    public void sendRequest(String page) {
        long time =System.currentTimeMillis();
        String url= URLHelper.messagelistsURL+
    ...
    protected void parseData(JSONObject response) {
        if (contentCode == Consts.ContentCode.Success) {
            try {
                ...
                MessageListsBean bean = parseJsonObject(dataJson.toString(), MessageListsBean.class);
                responseData = bean;
