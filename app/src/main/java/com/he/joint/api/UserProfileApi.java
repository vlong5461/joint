...
public class UserProfileApi  extends BaseApi {
    ...
    public void sendRequest(String user_id,String user_token) {
        long time =System.currentTimeMillis();
        String url= URLHelper.usereProfileURL+
    ...
    protected void parseData(JSONObject response) {
        if (contentCode == Consts.ContentCode.Success) {
            try {
                ...
                UserProfileBean bean = parseJsonObject(dataJson.toString(), UserProfileBean.class);
                responseData = bean;
