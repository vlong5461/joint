...
public class ExpertDetailApi extends BaseApi {

    public void sendRequest(String expert_id) {
        long time =System.currentTimeMillis();
        String url= URLHelper.expertDetailURL+
                "?expert_id="+expert_id+"&token=" + Algorithm.getTokenMD5(time)+"&timestamp=" + time;
        getStringData(url);
    }
    ...
    protected void parseData(JSONObject response) {
        if (contentCode == Consts.ContentCode.Success) {
            try {
                ...
                ExpertDetailBean bean = parseJsonObject(dataJson.toString(), ExpertDetailBean.class);
                responseData = bean;
