...
public class OpenUDID {
	...
	private static Context openContext = null;
	...
	public static void syncContext(Context mContext) {
		if (_openUdid == null) {
			try {
				openContext = mContext.createPackageContext("com.gf.rruu", Context.CONTEXT_IGNORE_SECURITY);
	...
	public static String getOpenUDID() {
		...
		// if (!CommonUtils.isEmpty(_openUdid) && _openUdid.contains(":")) {
		// return _openUdid.split(":")[1];
		// }
		if(openContext!=null){
			SharedPreferences mPreferences = openContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
			_openUdid = mPreferences.getString(PREF_KEY, null);
		}
		if(isMACSame(_openUdid)){
			if(openContext!=null) {
				generateImeiId(openContext);
			}else{
				generateRandomNumber();
			}
		}
		return _openUdid.replaceAll(":", "");
	}
	...
	private static void generateImeiId(Context mContext) {
		...
		try {
			...
			if (null != szImei && !szImei.substring(0, 3).equals("000")&& !szImei.equals("")) {
	...
	private static boolean isMACSame(String mac){
		String wifi=mac;
		wifi=wifi.replaceAll("WIFIMAC","");
		wifi=wifi.replaceAll(":","");
		wifi=wifi.replaceAll("-","");
		wifi=wifi.replaceAll("0","");
		if(wifi.length()>2){
			return false;
		}else {
			PreferenceHelper.saveToSharedPreferences(Consts.DeviceID_Before_V6, mac.replaceAll(":",""));
			return true;
		}
	}
	...
	private static void generateRandomNumber(Context mContext) {
		_openUdid = Md5(UUID.randomUUID().toString());
		SharedPreferences mPreferences = mContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
		Editor e = mPreferences.edit();
		e.putString(PREF_KEY, _openUdid);
	...
	public static void generateSystemId() {
		...
		String fp = String.format("%s/%s/%s/%s:%s/%s/%s:%s/%s/%d-%s-%s-%s-%s", Build.BRAND, Build.PRODUCT, Build.DEVICE,
				Build.BOARD, Build.VERSION.RELEASE, Build.ID, Build.VERSION.INCREMENTAL, Build.TYPE, Build.TAGS, Build.TIME,
